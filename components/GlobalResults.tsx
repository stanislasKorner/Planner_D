...
export const GlobalResults: React.FC<Props> = ({ attractions, allRankings, currentUser, myOrder }) => {
  ...
  const confirmReset = async () => {
    try { setIsResetting(true); await resetAllRankings(); setShowResetModal(false); } 
    catch (e) { alert("Erreur lors de la réinitialisation."); } 
    finally { setIsResetting(false); }
  };

  const mapSidebarList = optimizedPath.length > 0 ? optimizedPath.map(id => attractions.find(a => a.id === id)).filter(Boolean) as Attraction[] : topForAI;

  if (allRankings.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-slate-400 relative">
        {currentUser === 'Raphaël' && <button onClick={openResetModal} className="absolute top-0 right-0 text-xs font-bold text-red-200 hover:text-red-500 transition-colors">RESET</button>}
        {showResetModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 text-left">
                ...
                <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 p-6">
                    ...
                    <div className="flex gap-3 mt-6"><button onClick={() => setShowResetModal(false)} className="flex-1 py-3 rounded-xl font-bold text-slate-700 bg-slate-100">Annuler</button><button onClick={confirmReset} className="flex-1 py-3 rounded-xl font-bold text-white bg-red-600">Oui</button></div>
                </div>
            </div>
        )}
        <Sparkles className="w-12 h-12 mb-4 opacity-20" /><p className="font-medium">Aucun vote pour le moment</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto pb-20 pt-4 relative">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-2 text-emerald-800 font-bold text-sm uppercase tracking-wide"><CheckCircle2 size={16} /> Ont voté ({votersList.hasVoted.length})</div>
                <div className="flex flex-wrap gap-2">{votersList.hasVoted.map(name => <span key={name} className="bg-white text-emerald-700 px-2 py-1 rounded-md text-xs font-bold shadow-sm border border-emerald-100">{name}</span>)}</div>
            </div>
            <div className="bg-rose-50 border border-rose-100 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-2 text-rose-800 font-bold text-sm uppercase tracking-wide"><XCircle size={16} /> En attente ({votersList.notVoted.length})</div>
                <div className="flex flex-wrap gap-2">{votersList.notVoted.map(name => <span key={name} className="bg-white text-rose-700 px-2 py-1 rounded-md text-xs font-bold shadow-sm border border-rose-100 opacity-60">{name}</span>)}</div>
            </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 px-2">
           <div>
             <div className="flex items-center gap-3">
                ...
                {currentUser === 'Raphaël' && <button onClick={openResetModal} className="flex items-center gap-1 px-3 py-1 bg-red-50 text-red-600 rounded-full text-[10px] font-bold hover:bg-red-100 transition-colors border border-red-100"><Trash2 size={12} /> RESET</button>}
             </div>
        ...
        {activeTab === 'list' && (
            <div className="space-y-5 max-w-2xl mx-auto">
                {globalRanking.map((attraction, index) => {
                    ...
                    return <AttractionCard key={attraction.id} attraction={attraction} index={index} isDraggable={false} myRank={myRank > 0 ? myRank : undefined} topOneCount={topOneVoters.length} topOneVoters={topOneVoters} />;
                })}
            </div>
        )}
        ...
        {activeTab === 'map' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[700px]">
                ...
                <div className="lg:col-span-8 h-full rounded-3xl overflow-hidden shadow-sm border border-slate-100 bg-slate-50"><MapVisualization path={optimizedPath.length > 0 ? optimizedPath : topForAI.map(a => a.id)} attractions={attractions} hoveredId={hoveredAttractionId} /></div>
            </div>
        )}
